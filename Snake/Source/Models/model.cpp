...
void Model::setMoveDirection(const glm::vec3& moveDirection)
{
	if (_moveDirection != moveDirection)
	{
		if (_moveDirection == Settings::UP)
			if (moveDirection == Settings::RIGHT) rotateRight();
			else rotateLeft();

		if (_moveDirection == Settings::DOWN)
			if (moveDirection == Settings::RIGHT) rotateLeft();
			else rotateRight();

		if (_moveDirection == Settings::RIGHT)
			if (moveDirection == Settings::UP) rotateLeft();
			else rotateRight();

		if (_moveDirection == Settings::LEFT)
			if (moveDirection == Settings::UP) rotateRight();
			else rotateLeft();
	}
